...
S.UI = (function () {
    ...
    function performAction(value) {
        ...
        timedAction(function (index) {
            ...
            switch (action) {
                case 'countdown':
                    // The timer has begun: close the Pavi loading overlay now.
                    hidePaviLoading();
...
function checkBookFinished() {
    ...
    if (currentPage === lastPageIndex && lastPage && lastPage.classList.contains('flipped')) {
        if (!isBookFinished) {
            ...
            // Start the supplied background music as soon as the final page is reached.
            // This is called from the page-turn flow, so it has the best chance of
            // satisfying mobile browser media-gesture rules.
            ...
            // Combined experience handoff: notify the parent page only after
            // Pavi's final page has been completed. The second experience is
            // isolated in an iframe, so its CSS/JS cannot conflict with Pavi.
            setTimeout(() => {
                window.dispatchEvent(new CustomEvent('pavi:complete'));
            }, 1500);
        }
    }
}
...
// Used by the combined page handoff to stop Pavi's music cleanly.
