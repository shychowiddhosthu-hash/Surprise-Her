...
S.UI = (function () {
    ...
    function performAction(value) {
        ...
        // Hide the Pavi loading screen exactly when the countdown starts.
        function hidePaviLoading() {
            const loading = document.getElementById('pavi-loading');
            if (loading && !loading.classList.contains('hide')) {
                loading.classList.add('hide');
                // Remove it after the fade-out so it cannot block clicks or animation.
                setTimeout(() => {
        ...
        timedAction(function (index) {
            ...
            switch (action) {
                case 'countdown':
                    // The timer has begun: close the Pavi loading overlay and
                    // start the single Pavi BGM from the beginning.
                    hidePaviLoading();
                    startPaviMusicFromBeginning();
    ...
    if (book && bookContainer) {
        ...
        requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
                ...
                // Music is controlled only by the countdown and the visible
                // pause/play button. Opening the book must never restart it
                // or override a user's pause.
            });
        });
    }
...
function createHeartPhotoCentered(idx, total, onComplete) {
    if (heartPhotosCreated >= maxHeartPhotos) return false;
...
function showSurpriseButton() {
    let overlay = document.getElementById('pavi-surprise-overlay');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'pavi-surprise-overlay';
        overlay.innerHTML = `
            <div class="pavi-surprise-card">
                <div class="pavi-surprise-title">Here is the surprise ❤️</div>
                <button id="pavi-surprise-button" type="button">✨ Open Your Surprise</button>
            </div>`;
        document.body.appendChild(overlay);
    }

    overlay.classList.add('show');
    const button = document.getElementById('pavi-surprise-button');
    if (button && !button.dataset.bound) {
        button.dataset.bound = '1';
        button.addEventListener('click', () => {
            overlay.classList.remove('show');
            window.dispatchEvent(new CustomEvent('pavi:surprise'));
        });
    }
}
...
function spawnHeartPhotosCentered() {
    ...
    const total = Math.min(maxHeartPhotos, Math.max(photoUrls.length, 1) === 1 ? maxHeartPhotos : maxHeartPhotos);
    let currentIndex = 0;
    ...
    function spawnNext() {
        ...
        createHeartPhotoCentered(currentIndex, total, maybeComplete);
        currentIndex++;
...
function checkBookFinished() {
    ...
    if (currentPage === lastPageIndex && lastPage && lastPage.classList.contains('flipped')) {
        if (!isBookFinished) {
            ...
            // Do not advance automatically. The heart-photo sequence will show
            // the surprise button only after the final heart photo has finished.
        }
    }
}
...
window.startPaviMusicFromBeginning = function () {
    try {
...
// Kept only for backward compatibility. It deliberately does nothing so the
// combined handoff cannot interrupt or restart the soundtrack.
window.stopPaviMusic = function () {};

birthdayAudio.volume = 0.75;
birthdayAudio.src = 'music/birthday.mp3';
birthdayAudio.load();
...
function toggleMusic() {
    if (birthdayAudio.paused) {
        birthdayAudio.play().then(() => {
            ...
            // Browser autoplay policy: the visible control remains available for a user tap.
            isPlaying = false;
            musicControl.innerHTML = '▶';
            musicControl.classList.remove('playing');
        });
...
// Music is intentionally not force-started from book gestures.
// This preserves the user's pause/play choice.
